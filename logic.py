...
class ServerLogic:
    ...
    def do_server_logic(self, url, path, params):
        ...
        if file_name == "comet.js":
            ...
            if (file_name == "mediaQuery.js"):
                ...
                self.proceed_txt = self.ca.m.getMediaResponse(params[0][1], params[1][1], params[2][1], params[3][1])
    ...
    def handle_address_update(self, address):
        ...
        self.proceed_txt = "moveToAddress(" + str(swlat) + "," + str(swlng) + "," + str(nelat) + "," + str(nelng) + ",'" + longname + "');"
        self.ca.ajaxServer.stop()
        self.ca.cometServer.stop()
        self.ca.browser.load_uri("javascript:" + self.proceed_txt + ";void(0);")
    ...
    def handlePostAdd( self, rec  ):
        self.proceed_headers.append( ("Content-type", "text/javascript") )
        self.proceed_txt = "postAddMedia(" + rec.latitude + ", " + rec.longitude + ", '" + rec.getThumbUrl() + "', '" + rec.getThumbBasename() + "', '" + rec.tags + "');"
        self.ca.ajaxServer.stop()
        self.ca.cometServer.stop()
        self.ca.browser.load_uri("javascript:" + self.proceed_txt+";void(0);")
    ...
    def handleSavedMap( self, lat, lng, zoom, info ):
        ...
        if(info.find("Describe the map") != 0):
            self.proceed_txt = "setMap2(" + lat + "," + lng + "," + zoom + ",'" + urllib.quote(info) + "');"
        else:
            self.proceed_txt = "setMap2(" + lat + "," + lng + "," + zoom + ",'');"
        self.ca.ajaxServer.stop()
        self.ca.cometServer.stop()
        self.ca.browser.load_uri("javascript:" + self.proceed_txt + ";void(0);")
    ...
    def handleAddMarker( self, lat, lng, pixString, icon ):
        if(self.addKMLSet < 1):
            ...
            if(self.addKMLSet == -1):
                self.addKMLSet = 1
        self.proceed_txt = self.proceed_txt + "addInfoMarker(" + lat + ", " + lng + ", '" + pixString.replace("'",'"') + "', '" + icon + "',false);"
    ...
    def handleLine(self,id,color,thickness,pts):
        ...
        self.proceed_txt = self.proceed_txt + "addLine('" + id + "','" + color + "','" + thickness + "','" + pts + "');"
        self.ca.ajaxServer.stop()
        self.ca.cometServer.stop()
        self.ca.browser.load_uri("javascript:" + self.proceed_txt + ";void(0);")
