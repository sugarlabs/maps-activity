...
def load_svg(data, stroke, fill):
    if stroke is None or fill is None:
...
def get_unique_filepath(path, i):
    ...
    new_path = os.path.join(
        os.path.dirname(path_ob), str(
            str(i) + os.path.basename(path_ob)))
...
def gray_scale_pixbuf2(pb, copy):
    ...
    return GdkPixbuf.Pixbuf.new_from_data(
        arr, pb.get_colorspace(), pb.get_bits_per_sample())
